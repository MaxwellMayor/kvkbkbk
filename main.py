...
def popular_articles():
  ...
  for index,row in output.iterroows():
   m_data={
    "url": row['url'], "title": row['title'], "text": row['text'], "lang": row['lang'], "total_events": row['total_events']

    
  } 
   popular_article_data.append(m_data)
   return jsonify({
...
def recommended_articles():
  ...
  for index,row in all_recommended.iterroows():
   m_data={
    "url": row['url'], "title": row['title'], "text": row['text'], "lang": row['lang'], "total_events": row['total_events']

    
  } 
   recommended_article_data.append(m_data)
   return jsonify({
